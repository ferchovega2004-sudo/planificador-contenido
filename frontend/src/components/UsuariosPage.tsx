                  ...
                  <tr key={usr.id}>
                        ...
                        <div style={{ fontSize: '11px', color: 'var(--neon-cyan)', marginTop: '4px' }}>
                          ...
                          {usr.rol === 'ACOMPAÑANTE' && (
                            <span style={{ color: usr.activo === true ? '#10b981' : '#f59e0b', marginLeft: '8px', fontWeight: 'bold' }}>
                              ({usr.activo === true ? '👁️ Ver Detalles' : '🔒 Sin Detalles'})
                            </span>
                          )}
                        </div>
                      )}
                    ...
                    <td style={{ textAlign: 'right' }}>
                      {usr.rol === 'ACOMPAÑANTE' && (
                        <button
                          onClick={async () => {
                            try {
                              ...
                              setSuccessMsg(`Visualización de detalles para ${usr.nombre} cambiado a: ${nuevoEstado ? 'Habilitado' : 'Deshabilitado'}`);
                              cargarUsuarios();
                            } catch (err: any) {
                        ...
                        >
                          {usr.activo === true ? '🔒 Deshabilitar Ver Detalles' : '🔓 Habilitar Ver Detalles'}
                        </button>
                      )}
                      {usr.rol !== 'ADMIN' && (
                        <button
